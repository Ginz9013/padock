...
5. **Phase 4 — Deepen each domain** (sequenced one at a time, not simultaneously): ~~channels/threads for chat~~ **done** (§5.1.4), ~~configurable workflows for tasks~~ **done** (§5.1.5) — block-based editing for docs (Markdown → blocks migration) still pending.
