...
### 5.1.16 Docs tab: BlockNote editor + optimistic-lock autosave (done)

Replaces §5.1.12's plain title-input + Markdown `<textarea>` editor. The Doc detail page (`/projects/[id]/docs/[docId]`) now renders **BlockNote** (`@blocknote/core`/`@blocknote/react`/`@blocknote/shadcn`) as the block-style editing surface — closest available open-source match to Notion's own editing feel — with debounce autosave (1.5s idle) replacing the manual Save button.

- **BlockNote is a view/edit layer only — `Doc.blocks` keeps storing mdast (§5.1.7), not BlockNote's own block JSON.** Considered and rejected: making `Doc.blocks` store BlockNote's native block schema directly, which would avoid a parse/serialize step on every load/save. Rejected because BlockNote's block JSON is an unversioned internal representation of a fast-moving third-party package, not a stable spec the way mdast/remark is — coupling self-hosted, long-lived Doc data to it would tie every future `@blocknote/core` upgrade to a possible data migration, a cost mdast's stability doesn't carry. It would also invert Padock's own agent-first premise (§1/§5.1.3): Markdown would become a lossy derivative of the true stored state instead of being the state itself. The API is unchanged as a result — `packages/api/src/router/doc.ts` still only ever emits/accepts Markdown `content`; conversion (`tryParseMarkdownToBlocks`/`blocksToMarkdownLossy`) happens entirely inside the web UI at the load/save boundary — once per page open, once per debounce tick, not per keystroke — using `@blocknote/server-util` where the conversion needs to run outside the browser and `@blocknote/core`'s editor instance where it doesn't.
- **A custom editor engine (hand-rolled ProseMirror/contentEditable) was considered and rejected.** Taming `contentEditable` — selection across blocks, IME composition, paste sanitization, undo/redo — is the specific, well-known hard problem ProseMirror itself exists to solve; Plane (§0's own named reference) builds its Pages editor on Tiptap rather than a bespoke engine for the same reason. Because the editor is already just a swappable view layer over mdast (previous bullet), there's no lock-in cost to depending on a library here, so there was no offsetting reason to pay that cost.
- **Optimistic locking on `doc.update`**: a new optional `expectedUpdatedAt` input — when provided and it doesn't match the row's current `updatedAt`, the mutation throws `TRPCError({ code: "CONFLICT" })` instead of silently overwriting. The web UI's autosave always sends it, surfacing a "this doc changed elsewhere" banner on conflict with an explicit reload-vs-overwrite choice — neither side's edits are ever discarded without the user choosing that. CLI/MCP callers omit it, preserving today's fire-and-forget write behavior for agents. This still covers the motivating scenario — a human editing in the browser while an agent writes the same doc via CLI concurrently — because it's the *browser's* next autosave tick that carries a now-stale `expectedUpdatedAt` and gets caught; only two agents racing each other with neither specifying the field stays unprotected, which is pre-existing behavior, not a new regression.
...
| Doc editor (front-end) | BlockNote as a swappable view/edit layer over mdast, not a new storage format; custom-built editor engine rejected | §5.1.16. mdast stays the DB source of truth (agent-first, stable spec, no third-party schema lock-in); building a rich-text engine from scratch pays the industry's hardest contentEditable problems for no offsetting benefit once the editor is already swappable |
